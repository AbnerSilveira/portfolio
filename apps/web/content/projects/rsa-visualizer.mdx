...
Em desenvolvimento local, sobe o visualizador com `pnpm dev --filter=@projects/rsa-visualizer` (porta **3001** por omissão). O portfólio (`apps/web`) usa `NEXT_PUBLIC_RSA_VISUALIZER_URL` para apontar o iframe; sem variável, assume `http://localhost:3001`.
...
Código-fonte no monorepo, em `apps/projects/rsa-visualizer`.
